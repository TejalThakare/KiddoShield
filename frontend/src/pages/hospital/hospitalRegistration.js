...
export default function HospitalRegistration() {
  ...
  const signUpHandler = async (e) => {
    e.preventDefault();
    if (!/^[a-zA-Z\s]+$/.test(formdetails.hname)) {
      swal("Enter a valid name. Only letters and spaces are allowed.");
      return;
    }
    //validate password
    if (
    ...
    // Validate mobile number
    if (!/^\d{7,15}$/.test(formdetails.emergencynum)) {
      swal("Mobile number should be between 7 and 15 digits.");
      return; // Exit function if validation fails
    }
    ...
    <>
      ...
      >
        ...
        <ul className="nav__links">
          <li className="nav__item">{/* event listner to open modal */}</li>
        </ul>
      </nav>
      <h1
        id="header"
